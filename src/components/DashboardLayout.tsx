...
const DashboardLayout = ({ children, role, userName = "User" }: DashboardLayoutProps) => {
  ...
  return (
    <div className="flex min-h-screen dashboard-theme bg-white text-foreground">
      {/* Sidebar */}
      <aside className="fixed left-0 top-0 z-40 flex h-screen w-72 flex-col bg-[#050505] text-white border-r border-[#1a1a1a] shadow-2xl transition-all duration-300">
        {/* Gold Top Accent */}
        ...
        {/* User Identity Section - Premium Minimal */}
      ...
      {/* Main content */}
      <main className="ml-72 flex-1 bg-white min-h-screen relative flex flex-col">
        {/* Premium Top Bar */}
        <div className="sticky top-0 z-30 flex items-center justify-between border-b border-slate-100 bg-white/90 backdrop-blur-md px-10 py-4">
          ...
          <div className="flex items-center gap-6">
            <NotificationBell />
            <div className="h-10 w-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shadow-sm group hover:border-[#D4AF37] transition-colors">
        ...
        {/* Page content */}
        <div className="flex-1 p-8 lg:p-12 animate-in fade-in slide-in-from-bottom-2 duration-500">
          {children}
        </div>
      </main>
    </div>
